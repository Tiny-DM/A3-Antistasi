...
} forEach (_jna_datalist select IDC_RSCDISPLAYARSENAL_TAB_PRIMARYWEAPON);
...
} forEach (_jna_datalist select IDC_RSCDISPLAYARSENAL_TAB_HANDGUN);

if (count A3A_specialGrenadeLaunchers > 0) then {
    // muzzle + base grenade launchers, broken down in the arsenal
    private _rifleHM = createHashMapFromArray (_jna_datalist select IDC_RSCDISPLAYARSENAL_TAB_PRIMARYWEAPON);
    private _muzzleHM = createHashMapFromArray (_jna_datalist select IDC_RSCDISPLAYARSENAL_TAB_ITEMMUZZLE);
    {
...
} forEach (_jna_datalist select IDC_RSCDISPLAYARSENAL_TAB_SECONDARYWEAPON);
...
} forEach (_jna_datalist select IDC_RSCDISPLAYARSENAL_TAB_VEST);
...
private _uheadgear = [];
{
    ...
    [_array, _class, _amount] call _fnc_addItem;
} forEach (_jna_datalist select IDC_RSCDISPLAYARSENAL_TAB_HEADGEAR);
...
} forEach (_jna_datalist select IDC_RSCDISPLAYARSENAL_TAB_BACKPACK);
...
} forEach (_jna_datalist select IDC_RSCDISPLAYARSENAL_TAB_NVGS);

_rebelGear set ["NVGs", _nvgs];

// Unfiltered stuff (just radios atm? could add GPS)
private _radios = [];
{ [_radios, _x#0, _x#1] call _fnc_addItem } forEach (_jna_datalist select IDC_RSCDISPLAYARSENAL_TAB_RADIO);
...
} forEach (_jna_datalist select IDC_RSCDISPLAYARSENAL_TAB_CARGOMISC);
...
} forEach (_jna_datalist select IDC_RSCDISPLAYARSENAL_TAB_CARGOTHROW);
...
} forEach (_jna_datalist select IDC_RSCDISPLAYARSENAL_TAB_CARGOPUT);
...
} forEach (_jna_datalist select IDC_RSCDISPLAYARSENAL_TAB_ITEMOPTIC);
...
} forEach (_jna_datalist select IDC_RSCDISPLAYARSENAL_TAB_ITEMACC);
